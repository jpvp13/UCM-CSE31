...
int newBars(int initial,int n){
    ...
    if(bars == 0){         //this will run a check to find out when the user is out of money/wrappers
      ...
      newAmount = bars + newBars(bars, n);                  //this adds the ammount of current bars plus adding it recursively to totAmount
      return newAmount;       //returns total number of bars via exchange of wrappers for candies to maxBars
    }
  }
