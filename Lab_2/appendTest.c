...
char* append (char s1[ ], char s2[ ]) {
	...
	s1[k+s1len]  = '\0'; // by setting the very last index that uses s1 as a pointer to, we can reset the memory every time a new line is parsed back(if need be)
